...
struct Type1Kind {
    ...
    /// Maps glyph name → GlyphId using the font's actual charstring indices.
...
impl Type1Kind {
    ...
    fn map_code(&self, code: u8) -> GlyphId {
        if let Some(entry) = self.encodings.get(&code) {
            ...
            match self.encoding {
                Encoding::BuiltIn => self.font.table().encoding().and_then(|e| e.glyph_id(code)),
                _ => self
                    .encoding
                    .map_code(code)
    ...
    fn code_to_ps_name(&self, code: u8) -> Option<&str> {
        self.encodings
            ...
            .or_else(|| match self.encoding {
                Encoding::BuiltIn => self.font.table().encoding().and_then(|e| e.glyph_name(code)),
        ...
        if let Some(entry) = self.encodings.get(&code) {
            ...
            match self.encoding {
                Encoding::BuiltIn => {
                    ...
                    let name = table.glyph_name(gid)?;
                    // Latin1String bytes are valid UTF-8 for ASCII glyph names.
                    std::str::from_utf8(name.bytes()).ok()
                }
                _ => self.encoding.map_code(code),
            }
        }
